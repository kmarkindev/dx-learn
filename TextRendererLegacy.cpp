...
TextRendererLegacy::TextRendererLegacy(HWND hwnd, ID3D11Device* device, ID3D11DeviceContext* context,
...
void TextRendererLegacy::LoadFtFace(FT_Face* face, const char* filename, int faceIndex)
{
...
void TextRendererLegacy::LoadFtChar(FT_Face face, char symbol)
{
    if (FT_Load_Char(face, symbol, FT_LOAD_RENDER))
        throw std::runtime_error("Cannot load font char");
}

void TextRendererLegacy::CreateTextures(std::string_view text)
{
    for (char chr: text)
    {
        ...
        textureDesc.Width = glyph->bitmap
            .width;
        textureDesc.Height = glyph->bitmap
            .rows;
        ...
        textureDesc.SampleDesc
            .Count = 1;
        textureDesc.SampleDesc
            .Quality = 0;

        D3D11_SUBRESOURCE_DATA data = {};
        data.pSysMem = glyph->bitmap
            .buffer;
        data.SysMemPitch = glyph->bitmap
            .pitch;
        ...
        TextRendererLegacy::Letter letter = {
            ...
            static_cast<unsigned int>(glyph->advance
                .x)
        };
...
void TextRendererLegacy::CreateStates()
{
...
void TextRendererLegacy::CreateInputLayout(ID3DBlob* bytecode)
{
...
void TextRendererLegacy::CreateLetterMeshBuffer()
{
    using namespace DirectX;

    TextRendererLegacy::Vertex verts[] = {
        //first triangle
...
void TextRendererLegacy::CreateShadersAndInputLayout()
{
    ...
    CreateInputLayout(vertexShader.second
        .p);
...
void TextRendererLegacy::RenderString(std::string_view text, DirectX::XMFLOAT2 position, float scale)
{
    CreateTextures(text);
    ...
    for (char chr: text)
    {
        ...
        DirectX::XMFLOAT2 letterPos = {
            static_cast<float>(x += letter.bearing
                .x * scale),
            static_cast<float>(position.y - letter.bearing
                .y * scale)
        };
        ...
        UINT offset = 0;
        UINT stride = sizeof(TextRendererLegacy::Vertex);
